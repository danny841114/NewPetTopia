...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Integer> {
    ...
    @Query("""
            SELECT pd AS productDetail, SUM(od.quantity) AS totalQuantity
            FROM OrderDetail od
            JOIN od.product p
    ...
    @Query("""
            SELECT pc.name AS categoryName, SUM(od.quantity) AS totalQuantity
            FROM OrderDetail od
            JOIN od.product p
            JOIN p.productDetail pd
    ...
    @Query("""
            ...
            WHERE od.order.createdTime BETWEEN :startDate AND :endDate
            """)
