        ...
        try {
            ...
            if (existingUser != null) {
                ...
                Oauth2LoginResponse response = Oauth2LoginResponse.builder()
                        ...
                        .role(existingUser.getUserRole().toString())
                        .provider(providerEnum.toString())
                        .picture(request.getPicture())
                        .avatar(request.getPicture())
                        .hasVendorAccount(request.getHasVendorAccount())
                        .message("OAuth2 登入成功")
                        .build();

                logger.info("返回的用戶資訊: {}", response);
            ...
            Oauth2LoginResponse response = Oauth2LoginResponse.builder()
                    ...
                    .role(newUser.getUserRole().toString())
                    .provider(providerEnum.toString())
                    .picture(request.getPicture())
                    .avatar(request.getPicture())
                    .hasVendorAccount(false)
                    .message("OAuth2 註冊並登入成功")
                    .build();

            logger.info("返回的新用戶資訊: {}", response);
